   ...
   switch ($_GET['action']) {
      ...
      case 'hapus':
         ...
         if (isset($_GET['id'])) {
            ...
            $idadmin   = strip_tags(mysqli_real_escape_string($con, $_GET['id']));

            $sql   = $con->prepare("DELETE FROM t_admin WHERE id_admin = ?");
            $sql->bind_param('s', $idadmin);
            $sql->execute();
